...
def get_parser():
    ...
    parser.add_argument('--experiment', required=False, default="u-ensemble", type=lambda s: s.split(','), help="Type of experiment to run. [none/w-ensemble/u-ensemble/pdm/all]")
...
def train_net(net_id, net, train_dataloader, test_dataloader, epochs, lr, reg, reg_base_weights=None, device="cpu"):
...
def run_exp(n_nets):
    print("Current n_nets is %d " % n_nets)
    args.n_nets = n_nets
    ...
    if not os.path.exists(os.path.join(args.logdir, "n_nets "+str(n_nets))):
        mkdirs(os.path.join(args.logdir, "n_nets "+str(n_nets)))
    with open(os.path.join(args.logdir, "n_nets "+str(n_nets), 'experiment_arguments.json'), 'w') as f:
    ...
    for trial in range(args.trials):
        ...
        for net_id, net in nets.items():
            ...
            trainacc, testacc = train_net(net_id, net, train_dl, test_dl, args.epochs, args.lr, args.reg, device=device)
        ...
        if ("pdm" in args.experiment) or ("all" in args.experiment):
            ...
            best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_pdm_matching_multilayer(
                nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=args.iter_epochs, sigma=None, sigma0=None, gamma=None,
                device=device
            )
            ...
            trials_res[trial]["pdm_match"]["Best Result"] = {"Best Sigma0": best_sigma0, "Best sigma": best_sigma, 
                                                         "Best gamma": best_gamma, "Best Test accuracy": best_test_acc,
                                                         "Train acc": best_train_acc}

            trials_res[trial]["pdm_match"]["PDM log"] = str(res)                                                

        trials_res[trial]["pdm_KL_match"] = {}
        if ("pdm_KL" in args.experiment) or ("all" in args.experiment):
            print("Computing hungarian matching")
            best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_pdm_matching_multilayer(
                nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=args.iter_epochs, sigma=None, sigma0=None, gamma=None,
                device=device, KL=True
            )
            ...
            trials_res[trial]["pdm_KL_match"]["Best Result"] = {"Best Sigma0": best_sigma0, "Best sigma": best_sigma, 
                                                         "Best gamma": best_gamma, "Best Test accuracy": best_test_acc,
                                                         "Train acc": best_train_acc}
            trials_res[trial]["pdm_KL_match"]["PDM log"] = str(res)

        trials_res[trial]["pdm_KL2_match"] = {}
        if ("pdm_KL2" in args.experiment) or ("all" in args.experiment):
            print("Computing hungarian matching")
            best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_pdm_matching_multilayer(
                nets_list, train_dl, test_dl, traindata_cls_counts, args.net_config[-1], it=args.iter_epochs, sigma=None, sigma0=None, gamma=None,
                device=device, KL=2
            ...
            logging.debug("****** PDM_KL2 matching ******** ")
            logging.debug("Best Sigma0: %s. Best sigma: %s Best gamma: %s. Best Test accuracy: %s. Train acc: %s. \n"
                          % (str(best_sigma0), str(best_sigma), str(best_gamma), str(best_test_acc), str(best_train_acc)))

            logging.debug("PDM_KL2 log: %s " % str(res))

            trials_res[trial]["pdm_KL2_match"]["Best Result"] = {"Best Sigma0": best_sigma0, "Best sigma": best_sigma, 
                                                         "Best gamma": best_gamma, "Best Test accuracy": best_test_acc,
                                                         "Train acc": best_train_acc}
            trials_res[trial]["pdm_KL2_match"]["PDM log"] = str(res)
    ...
    with open(os.path.join(args.logdir, "n_nets "+str(n_nets), 'trials_res.json'), 'w') as f:
...
import contextlib
def abli_exp(n_nets=10, n_layers=1, dataset="mnist", partition='hetero-dir'):
    ...
    for partition in partitions:
        ...
        #                      dataset, partition)
        logdir = os.path.join('log_abli', '2020-08-17 16',
                              dataset, partition)
        args.logdir = logdir
        ...
        with contextlib.closing(Pool(args.num_pool_workers)) as po:
            pool_results = po.map_async(run_exp, (n_nets for n_nets in n_nets_list))
            results_list = pool_results.get()

        for n_nets, result in zip(n_nets_list, results_list):
            abli_res[n_nets] = result
...
if __name__ == "__main__":
    ...
    abli_exp(n_nets=args.n_nets, dataset=args.dataset)
